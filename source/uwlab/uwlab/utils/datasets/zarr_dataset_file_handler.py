...
class ZarrDatasetFileHandler(DatasetFileHandlerBase):
    ...
    def _convert_and_save_episode(self, episode: EpisodeData):
        ...
        for key, value in episode_dict.items():
            if key == "actions":
                episode_data[key] = value.cpu().numpy()
            elif key == "rewards":
                episode_data[key] = episode_dict.get("rewards", torch.zeros(num_frames)).cpu().numpy()
            elif key == "dones":
                episode_data[key] = episode_dict.get("dones", torch.cat([torch.zeros(num_frames - 1), torch.ones(1)])).cpu().numpy()
            elif isinstance(value, torch.Tensor):
                episode_data[key] = value.cpu().numpy()
            elif isinstance(value, dict) or isinstance(value, TensorDict):
                episode_data[key] = self._process_observations_for_episode(value)
            elif value is None:
                continue
            else:
        ...
        for key, value in episode_data.items():
            if isinstance(value, dict):
                for obs_key, obs_value in value.items():
                    self._extend_or_create_array(data_group, f"{key}/{obs_key}", obs_value, episode_length)
            else:
    ...
    def _is_image_array(self, data: np.ndarray) -> bool:
        """Check if array is an image array (4D with shape [T, C, H, W])."""
