...
_RSL_RL_SCRIPTS_DIR = Path(__file__).resolve().parents[3] / "scripts" / "reinforcement_learning" / "rsl_rl"
...
def _build_parser(config: dict[str, Any]) -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        default=bool(config.get("save_rendering", False)),
        help="Save env.render() viewport frames alongside camera images each step.",
    )
    parser.add_argument(
        "--camera_width", type=int, default=int(config.get("camera_width", 224)), help="Camera image width."
    )
    parser.add_argument(
        "--camera_height", type=int, default=int(config.get("camera_height", 224)), help="Camera image height."
    )
    parser.add_argument(
...
def _build_camera_cfgs(width: int = 224, height: int = 224) -> dict[str, TiledCameraCfg]:
    """Build TiledCameraCfgs for front, side, and wrist cameras (matching OctiLab ur5e setup)."""
    return {
        ...
        "side_camera": TiledCameraCfg(
            ...
            data_types=["rgb"],
            spawn=sim_utils.PinholeCameraCfg(focal_length=21.9),
        ),
...
def _capture_camera_images(env_unwrapped, camera_names: list[str]) -> dict[str, torch.Tensor]:
    """Read raw uint8 RGB images from TiledCamera scene sensors.

    Returns a dict mapping e.g. ``"front_rgb"`` -> ``(num_envs, H, W, 3)`` uint8 tensor.
    """
    images = {}
    for cam_name in camera_names:
        sensor = env_unwrapped.scene.sensors[cam_name]
        img = sensor.data.output["rgb"].clone()
        if img.shape[-1] == 4:
            img = img[..., :3]
        obs_key = cam_name.replace("_camera", "_rgb")
        images[obs_key] = img.to(dtype=torch.uint8)
    return images
...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg | DirectMARLEnvCfg, agent_cfg: RslRlBaseRunnerCfg) -> None:
    ...
    camera_cfgs = _build_camera_cfgs(args_cli.camera_width, args_cli.camera_height)
    for cam_name, cam_cfg in camera_cfgs.items():
    ...
    gym_env = _make_env_from_cfg(
        ...
        render_mode="rgb_array" if args_cli.save_rendering else None,
    )
    env = RslRlVecEnvWrapper(gym_env, clip_actions=agent_cfg.clip_actions)
    env_unwrapped = gym_env.unwrapped
    active_camera_names = list(camera_cfgs.keys())
    ...
    try:
        ...
        while collected < target:
            ...
            camera_images = _capture_camera_images(env_unwrapped, active_camera_names)
            ...
            for ei in range(num_envs):
                ...
                for cam_key, cam_imgs in camera_images.items():
                    trajectories[ei]["obs_images"].setdefault(cam_key, []).append(
                        _to_cpu_detached(cam_imgs[ei : ei + 1])
                    )
